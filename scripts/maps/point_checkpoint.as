...
class point_checkpoint : ScriptBaseAnimating
{
	...
	bool KeyValue( const string& in szKey, const string& in szValue )
	{
        ...
        else if( szKey == "m_lpszActivateTarget" )
        {
            m_lpszActivateTarget = szValue;
            return true;
        }
		else
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		// Set activated
		self.pev.frags = 1.0f;
		g_PlayerFuncs.ClientPrintAll( HUD_PRINTTALK, string( pOther.pev.netname ) + " just activated a Respawn-Point.\n" );
        ...
        //g_PlayerFuncs.ClientPrintAll(HUD_PRINTTALK, "m_lpszActivateTarget: " + m_lpszActivateTarget + "\n");
        
        if( m_lpszActivateTarget != "" && m_lpszActivateTarget != self.GetTargetname() )
		{
            CBaseEntity@ pEntity = null;
            while ((@pEntity = g_EntityFuncs.FindEntityByTargetname(@pEntity, m_lpszActivateTarget )) !is null) {
                pEntity.Use(@self, @self, USE_TOGGLE, 0.f);
            }
		}
	}
